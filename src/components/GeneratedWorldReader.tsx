...
export default function GeneratedWorldReader({ world, close }: { world: World; close(): void }) {
  ...
  const chooseCharacter = async (characterId: string) => {
    ...
    try {
      const result = await characterPerspective(world.id, characterId);
      ...
      void prepareChapterImages({
        ...
        onProgress: (completed, total) => setIllustrationProgress({ number: currentChapter?.number ?? 0, completed, total }),
      }).finally(() => setIllustrationProgress(null));
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "Perspective could not be generated.");
  ...
  const advance = async (authorCommand?: string) => {
    ...
    try {
      ...
      void prepareChapterImages({
        ...
        generate: generateSceneImage,
        onImage: (beat, image) => setStoredImages((current) => ({ ...current, [beat.id]: image })),
        onProgress: (completed, total) => setIllustrationProgress({ number: next.number, completed, total }),
      }).finally(() => setIllustrationProgress(null));
    } catch (reason) {
      ...
      : error && !story ? <div className="story-boot"><h1>The chapter is waiting.</h1><p>{error}</p><button type="button" className="enter-button" onClick={retryBootstrap}>Try again</button></div>
        : chapterGeneration ? <ChapterGenerationView generation={chapterGeneration} />
          : story && chapter ? <div className="generated-grid"><section className="generated-story">
            ...
            <p className="eyebrow">CHAPTER {chapter.number}</p><h1>{chapter.title}</h1><div className={`perspective-banner ${activeCharacter ? "perspective-banner--character" : ""}`}><span>NOW VIEWING</span><b>{activeCharacter ? `${readerLabel}'s perspective` : "StoryVerse narrator · canonical world view"}</b><p>{activeCharacter ? `This chapter is filtered through ${readerLabel}'s memories, goals, and observations.` : "This is the omniscient, shared version of events—not any single character’s point of view."}</p>{activeCharacter && <button type="button" className="canonical-return" onClick={() => showNarratorChapter(chapter)}>Return to canonical view →</button>}</div><ChapterBgm key={`bgm-${chapter.id}-${activeCharacter ?? "canonical"}`} worldId={world.id} chapterId={chapter.id} protagonistId={activeCharacter ?? undefined} chapterText={`${chapter.title}\n${viewNarration}`} perspective={activeCharacter ? `${readerLabel}'s POV` : "Canonical world view"} onPlan={setAudioPlan} /><ChapterNarration key={`narration-${chapter.id}-${activeCharacter ?? "canonical"}`} worldId={world.id} chapterId={chapter.id} protagonistId={activeCharacter ?? undefined} plan={audioPlan} /><p className="generated-state">{story.worldState}</p><p className="scene-prose">{viewNarration}</p>
            ...
            <div className="beat-sequence">{viewBeats.map((beat) => <article key={beat.id}><SceneImage image={storedImages[beat.id]} worldId={world.id} sceneId={beat.id} moment={moment} protagonistId={activeCharacter ?? undefined} title={beat.caption} description={beat.description} loadImage={isLatestChapter && !illustrationProgress ? loadImage : undefined} retryImage={(request) => generateSceneImage(request, true)} /><p>{beat.caption}</p></article>)}</div>
          ...
          </section><aside className="story-cast"><p className="panel-label">PERSISTENT CAST</p>{story.characters.map((character) => <button type="button" className={character.id === activeCharacter ? "active" : ""} key={character.id} disabled={busy || Boolean(illustrationProgress) || !isLatestChapter} onClick={() => void chooseCharacter(character.id)}><b>{character.name}</b><span>{character.role}</span><small>{character.personality}</small></button>)}<p className="cast-note">{isLatestChapter ? "Choose a character to see the current chapter through only their memories, goals, and observations." : "Character perspectives are available on the current chapter; this archived chapter keeps its original narration and illustrations."}</p></aside></div>
